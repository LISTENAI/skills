...
示例与触发语句见 `references/usage.md`（按需加载，避免主文件过长）。
...
- 查阅 `references/knowledge.md` 对应 topic 看是否有已知解决方案
...
每次执行结束后，在用户项目根目录的 `.arcs-dev-tools/reports/` 下生成报告（避免写入 skill 安装目录）：

**文件名**：`YYYY-MM-DD_<编号>.md`（如 `2026-02-25_001.md`），不含具体项目名。报告本质是配合 knowledge 更新的临时载体。

内容包括：执行摘要、遇到的问题（现象/是否偶发/根因/解决方案）、关键数据、建议。

### 经验知识库

文件：`references/knowledge.md`
...
- 去重合并优先，同类问题只更新次数
- 只有全新类型的问题才追加新条目
- 每个 topic 控制在 5-8 条以内
- **topic 满时**：提醒用户审查是否有可合并或淘汰的条目

### 自优化 SKILL.md

发现应沉淀到 skill 的通用性问题时：
1. **不要直接修改 SKILL.md**
2. 在执行报告末尾写 `## SKILL 优化建议`
3. 告知用户，**用户同意后**再更新 SKILL.md
